...
python src/train.py --train_file $train_file --dev_file $dev_file -lm $lm_ident -l $limit_train $cont -o ${out_fol}/models/ -str $strategy -bs $batch_size --patience $patience -lr $learning_rate -wd $weight_decay -mgn $max_grad_norm -ne $num_train_epochs -wr $warmup_ratio $adafactor -ls $label_smoothing -of ${out_fol}/output/ -pa $padding $grad_check -eas $eval_accumulation_steps --max_length $max_length --dropout $dropout --seed $seed $ignore_data > ${out_fol}/log/train.log
...
for test_file in $files_to_parse; do
    ...
    python src/parse.py -m ${out_fol}/models/ --train_log ${out_fol}/log/train.log --lm_ident $lm_ident --sent_file $test_file -o ${out_fol}/output/${out_prefixes[$count]} -pa $padding -ml $max_length > ${out_fol}/eval/${out_prefixes[$count]}.eval
    (( count++ )) || true
done
