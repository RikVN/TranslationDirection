...
# Do all our preprocessing and data handling
# See https://github.com/RikVN/TranslationDirection for more explanation

set -eu -o pipefail

# We will process the data to sentence-level and document-level.
# For document-level, **note** that we set a max length so it still fits in the model size of the LM we use.
# In our case XLM-R, which has at most 512 tokens.
# Giving some room for special tokens, we set a max of 500 tokens per document.
...
# Now in exp_data/europarl/full/ we have files such as bg-en.doc.tab with the Bulgarian original data and English translations at the document-level.
# There is also en-bg.doc.tab with the opposite translation direction.
# There is an issue: there is a lot more original English data for any pair that includes English.
# Similarly, some language pairs in general have a lot more data than other language-pairs.
# If we want to train a multi-lingual system, we probably want equal amounts of data for each language pair (as much as possible).
# So, we want to down-sample the data sets.
# First, we shuffle the original data sets and save them, so we always go back.
# Note that this way, we lose the order of the sentences/documents, but this happens anyway during training of system.
# We also want to deduplicate this data, as there are likely a lot of similar sentences (EU boilerplate).
# We use src/dedup.py to do this, which filters all near-duplicate sentences and documents.
...
# Before actually down-sampling, we have to think about dev and test sets.
# We have different test sets available from WMT, and also ones we selected ourselves from the MaCoCu project.
# But we also want to split off dev/test for each Europarl test.
# This also depends on which language pair you're interested in.
# In our case, we're interested in Bulgarian, so we take a larger dev/test part for this language.
# For the other languages, we keep 250 instances per file for the sentence-level, and 100 for the document-level.
...
# Still, these are not training sets, as they're all individual sets in one direction.
# Let's create training sets from all down-sampled sets.
# There's a problem though: in these data sets the first text is always the original.
# It's also still in order per language.
# If we are training a model, we want to randomize the first and second sentence so that there is no bias introduced in that regard.
...
# We also create down-sampled training splits again, which we put in the right format:
